...
| Barra lateral (Sidebar) | IMPLEMENTADO | — | Es chrome persistente (tema, atajos a Remisiones/Piezas General, logout, config, updater) — **no** es un menú de navegación entre pantallas. Remisiones y Piezas General son filas ícono+texto debajo del toggle de tema (con divisor compartido), Configuraciones sigue siendo el ícono cuadrado de abajo |
...
| Backups — local diario por usuario al entrar a la app | IMPLEMENTADO, **desactivado por defecto** por usuario (`users.backup_local_diario = 0`) | Otorgado por un admin en `UsersPanel` (no es un permiso de `PERMISOS`) | Máximo un backup por día por máquina, trackeado en `localStorage`, no en `backup_history`. Ver [DISASTER_RECOVERY.md](DISASTER_RECOVERY.md#backup-local-diario-por-usuario) |
