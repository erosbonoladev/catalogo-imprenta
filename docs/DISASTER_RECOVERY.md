...
1. **Disparado desde la app** (botón manual, hook previo a importaciones, backup de emergencia antes de restaurar): corre dentro del webview de Clio, usa `src/backup.ts` (lógica pura) + `src/db.ts` (`runBackupNow`), y guarda el archivo `.sql.gz` localmente bajo `appDataDir()/backups` de la máquina donde se disparó. `runBackupNow` solo lee las tablas (`readBackupTables`, contra `client`); armar el dump, validarlo y comprimirlo/hashearlo corre en un Web Worker (`src/backupWorker.ts`, orquestado desde `src/backupWorkerClient.ts`) — con `products.imagen`/`plastic_products.imagen` sumando ~150MB, hacerlo en el hilo de UI colgaba la app entera mientras corría (confirmado en producción: backups `BACKUP_LOCAL_DIARIO` fallidos con `RangeError: Invalid array length` y corridas que quedaron `EN_PROCESO`, señal de que alguien cerró la app a la fuerza mientras estaba colgada). El mismo motivo aplica a validar un archivo de restauración subido (puede pesar cientos de MB ya descomprimido) — corre en `src/restoreValidationWorker.ts` vía `validateRestoreFile()`.

`readBackupTables` en sí lee la mayoría de las tablas dentro de una transacción de solo lectura (para una foto consistente), pero **`products` y `plastic_products` se leen aparte, con una consulta suelta fuera de esa transacción** — medido contra Turso en producción: leer todo dentro de una sola transacción tardaba ~20s, más de la mitad solo esas dos tablas (imágenes). En una red más lenta que la de oficina, Turso cierra la transacción interactiva antes de llegar al `commit`, y el `rollback` del `catch` falla con `LibsqlError: SQLITE_UNKNOWN: cannot rollback - no transaction is active` porque ya no queda nada que deshacer (confirmado en producción con el aviso de backup diario). Una consulta suelta no arrastra ese límite de sesión aunque tarde lo mismo en transferir los mismos ~150MB. El costo: `products`/`plastic_products` pueden quedar unos segundos más viejas/nuevas que el resto del dump en un backup disparado desde la app — el automático programado (`clio-backups`, corre contra una red rápida) no tiene este problema y sigue leyendo todo con la misma transacción de siempre.
...
**Ya no es automático — es un aviso que el usuario confirma.** Hasta la 0.12.2 un admin marcaba a un usuario (`users.backup_local_diario`) para que el backup se disparara solo, en silencio, al entrar a la app. En producción eso resultó ser el origen de backups fallidos y de la app colgada: armar+comprimir+hashear el dump completo (imágenes de `products`/`plastic_products` incluidas, ~150MB) de forma síncrona apenas se entraba a Clio agotaba un límite de memoria del motor de JS en algunas máquinas (`RangeError: Invalid array length`, corridas que quedaban `EN_PROCESO` por cierres forzados) — y como corría sin que nadie lo pidiera, nadie se enteraba hasta que ya había pasado. Ver el commit de la 0.12.2 y versiones posteriores para el detalle. `users.backup_local_diario` sigue existiendo en el esquema (no se hizo `DROP COLUMN` sobre Turso en vivo) pero es una columna muerta — ningún código la lee ni la escribe, mismo tratamiento que cualquier otra columna muerta documentada en [DATABASE.md](DATABASE.md).

**Flujo actual** (`DailyBackupPrompt.tsx`, montado una vez en `App.tsx` — visible en cualquier pantalla mientras haya sesión): quien tenga el permiso `backups_crear` ve un modal ("Backup diario") la primera vez que entra a la app cada día en esa máquina, con dos botones — "Crear backup" o "Ahora no". El "una vez por día" se trackea en `localStorage` de esa máquina (namespaced por `user.id`), no en `backup_history` (compartida en Turso) — mismo motivo que antes: cada máquina donde esa persona entra necesita su propia copia local. Si el usuario pospone ("Ahora no") no se guarda nada en `localStorage`, así que el modal vuelve a aparecer en la siguiente apertura de la app ese mismo día — solo desaparece para el resto del día cuando el backup termina con éxito.

Al hacer clic en "Crear backup" corre `runBackupNow("BACKUP_LOCAL_DIARIO", ...)` (mismo tipo/mismo camino de `backup_history` que antes, ahora con `origen: "Notificación diaria"`) — mismo Web Worker que usa "Crear backup ahora" (ver "Arquitectura" arriba), así que aunque el dump sea pesado no bloquea la interfaz. Si falla, el error se muestra directo en el modal (no queda silencioso) y el modal sigue abierto para reintentar. Si tiene éxito: se guarda internamente en `appDataDir()/backups`, se marca "hecho por hoy" en `localStorage`, y justo después se abre el diálogo nativo "Guardar como" (`saveBackupFileAs`, mismo mecanismo que "Crear backup ahora") para que la persona elija además dónde quiere una copia en esa computadora (USB, carpeta compartida, etc.) — cancelar ese diálogo no afecta al backup, que ya quedó guardado y registrado igual.
