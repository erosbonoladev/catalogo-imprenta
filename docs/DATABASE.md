...
| `users` | `username` (unique), `password_hash` (bcrypt), `activo`, `rol` (`usuario`\|`admin`), `creado_en`, `failed_attempts`, `locked_until`, `session_token`, `session_expires_at` | Login con bloqueo y sesión por token con vencimiento deslizante — ver [PERMISSIONS.md](PERMISSIONS.md) |
...
- **Precios**: `upsertPrecio` (INSERT...ON CONFLICT, también escribe `precios_historial`; `tipo` es opcional en el input — si se omite al actualizar un SKU existente, conserva el que ya tenía vía `COALESCE(?, precios.tipo)`, nunca lo borra por una simple edición de precio), `getPrecio`, `getPreciosBySkuPrincipal`, `getPreciosList`, `searchPrecios` (SKU normal o con letra / nombre — usada por el buscador de `RemisionForm`, no por `searchProducts`).
