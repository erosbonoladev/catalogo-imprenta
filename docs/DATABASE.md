...
- **Remisiones**: `createRemisionConFolio` (folio + header + renglones en un solo `client.transaction()` — ver nota arriba), `listRemisiones`, `cancelRemision`, `listRemisionRenglonesParaHistorial` (join usado por la exportación "Historial de remisiones").
