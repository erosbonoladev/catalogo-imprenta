...
El **Sidebar no es un menú de navegación** — es chrome persistente (colapsar, usuario, toggle de tema, atajos a Remisiones/SKU Master/Piezas General gateados, logout, engranaje de Configuraciones gateado, `UpdateChecker`). Las transiciones entre pantallas siguen siendo props/callbacks (`onSelect`, `onBack`, `onOpenImprenta`, `onRemisiones`, `onSkuMaster`, `onPiezasGeneral`, `onConfiguraciones`, etc.) de cada pantalla — el Sidebar solo tiene cuatro "atajos" especiales (Remisiones, SKU Master, Piezas General y Configuraciones) que llaman de vuelta a `App.tsx`, no una lista general de destinos. Remisiones, SKU Master y Piezas General son filas con ícono+texto (`Assets/remisiones.svg`/`Assets/sku-master.svg`/`Assets/piezas.svg`) debajo del toggle de tema, en ese orden, compartiendo un solo `.sidebar-divider` (cada una se oculta independientemente según `remisiones_acceso`/`sku_master`/`plasticos`); Configuraciones sigue siendo el ícono cuadrado solo en `.sidebar-bottom-group` — no es el mismo patrón visual, a propósito (el usuario pidió esa ubicación específicamente para los atajos tipo Remisiones).
