...
El **Sidebar no es un menú de navegación** — es chrome persistente (colapsar, usuario, toggle de tema, atajos a Remisiones/SKU Master/Piezas General/Cortes de Madera gateados, logout, engranaje de Configuraciones gateado, `UpdateChecker`). Las transiciones entre pantallas siguen siendo props/callbacks (`onSelect`, `onBack`, `onOpenImprenta`, `onRemisiones`, `onSkuMaster`, `onPiezasGeneral`, `onCortesMadera`, `onConfiguraciones`, etc.) de cada pantalla — el Sidebar solo tiene cinco "atajos" especiales (Remisiones, SKU Master, Piezas General, Cortes de Madera y Configuraciones) que llaman de vuelta a `App.tsx`, no una lista general de destinos. Remisiones, SKU Master, Piezas General y Cortes de Madera son filas con ícono+texto (`Assets/remisiones.svg`/`Assets/sku-master.svg`/`Assets/piezas.svg`/`Assets/cortes-madera.svg`) debajo del toggle de tema, en ese orden, compartiendo un solo `.sidebar-divider` (cada una se oculta independientemente según `remisiones_acceso`/`sku_master`/`plasticos`/`maderas` — Cortes de Madera reusa el mismo permiso `maderas` que ya gatea `MaderasSection`, mismo criterio que `plasticos` cubriendo tanto `PlasticosSection` como `PiezasGeneralSection`); Configuraciones sigue siendo el ícono cuadrado solo en `.sidebar-bottom-group` — no es el mismo patrón visual, a propósito (el usuario pidió esa ubicación específicamente para los atajos tipo Remisiones).

## Instalaciones y actualizaciones

Antes del login de usuario hay un gate de **máquina**: `ActivationProvider` (`src/activationContext.tsx`) envuelve toda la app en `main.tsx`, por fuera de `UpdateProvider`/`AuthProvider`. Sin una instalación activada (credencial guardada en el Keychain/Credential Manager nativo vía `keyring`, comandos Rust en `src-tauri/src/lib.rs`) se muestra `ActivationScreen` y nada más se monta — ni el chequeo de actualizaciones ni `LoginScreen`. Detalle completo del sistema de credenciales/activación/revocación: [DISTRIBUTION.md](DISTRIBUTION.md).

Una vez autorizada la instalación (o dentro de la ventana de gracia offline de 7 días), `UpdateProvider` (`src/updateContext.tsx`) envuelve el resto de la app, así que el chequeo automático contra `@tauri-apps/plugin-updater` arranca apenas se monta — mientras se ve `LoginScreen`, no después del login — y el resultado ya está listo (o llega segundos después) cuando el usuario entra a la pantalla principal. El endpoint del updater (`tauri.conf.json` → `plugins.updater.endpoints`) ya no apunta directo a GitHub: apunta al Update API (Cloudflare Worker, `update-api/`), que autentica la instalación (headers `Authorization`/`X-Installation-Id`, agregados en `checkNow()`) antes de devolver el manifiesto, y solo el Worker tiene el PAT de GitHub necesario para leer el release privado. `UpdateChecker` (botón en `Sidebar`) consume ese mismo estado compartido en vez de chequear por su cuenta: si el chequeo silencioso ya encontró una actualización, un `useEffect` dispara el `Toast` de aviso ("Actualización disponible: vX.X.X") apenas se monta (es decir, apenas el usuario llega a la pantalla principal, porque `UpdateChecker` solo se monta logueado); si todavía estaba en curso, el aviso sale en cuanto termine. El botón sigue sirviendo para re-chequear a mano o, con una actualización ya detectada (badge visible), para instalarla (`downloadAndInstall` + `relaunch`).
...
Comandos (`src-tauri/src/lib.rs`): solo `hash_password`/`verify_password` (bcrypt puro) y `allow_fs_path`. Plugins: `dialog`, `fs`, `opener`, `updater`, `process`, `http`. Comandos custom (`#[tauri::command]`) no necesitan entrada en capabilities; los permisos de plugin sí (`src-tauri/capabilities/default.json`).

**Gotcha recurrente**: `<plugin>:default` casi nunca cubre todo lo que el plugin puede hacer — es el subconjunto "seguro". El permiso real hay que buscarlo en el `permissions/default.toml` propio del crate (fuente en crates.io / `~/.cargo/registry/src/`). Ya pasó tres veces: `sql:default` no traía `allow-execute` (solo lectura funcionaba); `fs:default` no cubre archivos fuera del sandbox de la app; `http:default` habilita el comando `fetch` pero con **scope vacío** — no autoriza ningún origen hasta que se declara explícitamente (`{"identifier": "http:default", "allow": [{"url": "..."}]}` en `capabilities/default.json`). Cambios de capability requieren rebuild completo (matar y reiniciar `tauri dev`), no alcanza con HMR.

**Plugin `http`** (agregado para la importación masiva de piezas, que descarga imágenes desde links de Google Drive/Photos en la columna "Links Imágenes Piezas" del Excel): el CSP del webview (`connect-src 'self' https://*.turso.io wss://*.turso.io`, en `tauri.conf.json`) bloquearía un `fetch()` normal del navegador a cualquier otro origen — `@tauri-apps/plugin-http` evita esto porque la petición HTTP corre en el proceso Rust, no en el webview, así que no la toca el CSP. El scope autorizado hoy en `capabilities/default.json` es solo `drive.google.com`, `docs.google.com`, `*.googleusercontent.com` y `photos.google.com` — si el origen real de los links cambia, hay que ampliar ese scope (nunca a `https://*` genérico) y volver a compilar.
...
- Token de Turso embebido en el bundle en build time — sin backend no hay dónde más guardarlo. Mitigación: token scopeado a una sola DB, rotable desde el dashboard de Turso. No intentar "ocultarlo mejor" en el bundle — es teatro de seguridad para una app de escritorio; una solución real sería un backend intermediario, que es un cambio de arquitectura, no un parche. El Update API (`update-api/`, ver [DISTRIBUTION.md](DISTRIBUTION.md)) es exactamente ese backend intermediario, pero solo para instalaciones/actualizaciones — no reemplaza este constraint para el resto de la app (catálogo/piezas/remisiones/etc. siguen yendo directo a Turso desde el cliente).
- Sin modo offline.
