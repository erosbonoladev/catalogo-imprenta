...
`UpdateProvider` (`src/updateContext.tsx`) envuelve toda la app en `main.tsx` (fuera de `AuthProvider`, junto a `ThemeProvider`), así que el chequeo automático contra `@tauri-apps/plugin-updater` arranca apenas carga la app — mientras se ve `LoginScreen`, no después del login — y el resultado ya está listo (o llega segundos después) cuando el usuario entra a la pantalla principal. `UpdateChecker` (botón en `Sidebar`) consume ese mismo estado compartido en vez de chequear por su cuenta: si el chequeo silencioso ya encontró una actualización, un `useEffect` dispara el `Toast` de aviso ("Actualización disponible: vX.X.X") apenas se monta (es decir, apenas el usuario llega a la pantalla principal, porque `UpdateChecker` solo se monta logueado); si todavía estaba en curso, el aviso sale en cuanto termine. El botón sigue sirviendo para re-chequear a mano o, con una actualización ya detectada (badge visible), para instalarla (`downloadAndInstall` + `relaunch`).
