...
PERMISOS = [
  ...
  "precios_venta_ver", "precios_venta_modificar",
...
| `precios_ver` | Precios Imprenta: ver | Botón "Precios Imprenta" junto al SKU en `ProductDetail` y apertura de `PreciosModal` — etiqueta de UI cambió de "Precios" a "Precios Imprenta" (2026-09-11), el identificador interno no cambió (no invalida asignaciones ya otorgadas), mismo precedente que `plasticos`/"Piezas" abajo |
| `precios_modificar` | Precios Imprenta: modificar | Dentro de `PreciosModal`: la tabla es de solo lectura hasta presionar "Editar" (visible solo con este permiso); sin el permiso, no hay botón "Editar" ni "Agregar nuevo producto" |
| `precios_venta_ver` | Precios Venta: ver | Botón "Precios Venta" junto al SKU en `ProductDetail` y apertura de `PreciosVentaModal` — dato nuevo (2026-09-11), deliberadamente **no** comparte permiso con Precios Imprenta: quien ya tenía `precios_ver` no ve Precios Venta automáticamente, un admin debe otorgarlo aparte |
| `precios_venta_modificar` | Precios Venta: modificar | Dentro de `PreciosVentaModal`: habilita el botón "Editar" (igual patrón view/edit que `PreciosModal`) sobre las 5 categorías fijas (`precios_venta`, ver [DATABASE.md](DATABASE.md)) |
...
| `instalaciones_ver` | Instalaciones: ver | Tab "Instalaciones" en `Configuraciones` (`InstalacionesPanel`) — lista de instalaciones activadas |
| `instalaciones_crear_credenciales` | Instalaciones: crear credenciales | Botón "Crear credencial" en `InstalacionesPanel` (también cubre "regenerar") |
| `instalaciones_revocar` | Instalaciones: revocar | Botón "Revocar instalación" en `InstalacionesPanel` |
| `instalaciones_reactivar` | Instalaciones: reactivar | Botón "Reactivar instalación" en `InstalacionesPanel` |
...
- `assertActorAuthorized(actor, requiredPermiso?)`: sesión vigente + (rol admin, o el/los permiso(s) indicados). `requiredPermiso` acepta un solo `Permiso` o un array — con array, basta con tener **cualquiera** de ellos. Usado por:
  ...
  - `savePreciosVenta` (`precios_venta_modificar`) — permiso propio, independiente de `precios_modificar`.
...
Las lecturas de precios/remisiones/backups/logs (financieras o sensibles) también exigen `Actor`: `getPrecio`, `getPreciosBySkuPrincipal`, `searchPrecios` (`precios_ver`, `precios_modificar`, `remisiones_acceso` o `remisiones_crear`); `getPreciosList` (`precios_ver`, `precios_modificar`, `sku_master` o `backups_ver`); `getPreciosVenta` (`precios_venta_ver` o `precios_venta_modificar`); `listRemisiones`, `getRemisionRenglones` (`remisiones_acceso`); `listRemisionRenglonesParaHistorial`, `listBackupHistory`, `getBackupSettings` (`backups_ver`); `getRecentLogs` (`configuraciones`) — mismo conjunto de permisos que ya exige la pantalla que las llama hoy, así que no restringe ningún flujo existente.

Esto no protege contra alguien que ya tiene el token de Turso y sabe qué SQL correr a mano, pero sí evita que un botón mal gateado, un bug de UI, o llamar estas funciones "a mano" desde la consola sin una sesión real de por medio ejecute la operación o lea estos datos.

**`instalaciones_*` es la única familia de permisos cuya autorización real no vive en `src/db.ts`**: las acciones administrativas (crear/regenerar credencial, listar/revocar/reactivar instalaciones) las ejecuta el Update API (Cloudflare Worker, `update-api/`, ver [DISTRIBUTION.md](DISTRIBUTION.md)), no `db.ts`. `update-api/src/logic/adminAuth.ts` (`assertActorAuthorized`) es una copia deliberada de `assertActorAuthorized`/`loadActorSession` de acá, pero corriendo contra un token **read-only** de la base principal (el Worker nunca tiene el token de escritura) — mismo criterio exacto (sesión vigente + activa; rol admin pasa siempre; si no, exige el/los permiso(s) indicado(s) en `user_permissions`). Si la lógica de sesión/permisos cambia en `src/db.ts`, hay que replicar el cambio a mano en `update-api/src/logic/adminAuth.ts` — no hay import compartido entre los dos proyectos. `runBackupNow` (crear backup) queda deliberadamente **fuera** de este check: la usan tanto el botón manual (gateado por `backups_crear` en la UI) como los hooks automáticos de pre-importación/pre-restauración, que deben poder correr sin importar si el usuario actual tiene ese permiso puntual — es una red de seguridad, no una acción discrecional.
