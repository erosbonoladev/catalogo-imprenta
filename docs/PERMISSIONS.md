...
| `configuraciones` | Configuraciones | `Configuraciones` (tabs Usuarios/Conectados/Registro/Captura) |
| `requisiciones` | Requisiciones | Botón "Requisición" por spec en `ProductDetail` |
| `backups_ver` | Backups: ver | Ver el tab "Backups" dentro de Configuraciones y su historial |
...
- **`isAdmin(user)`** (`auth.tsx`): gate independiente, más estricto, para pantallas que **no son otorgables como permiso normal** — son exclusivas del rol admin: `FichaImportPanel`, `ImageImportPanel`, y las dos pestañas de captura masiva dentro de `Configuraciones`.
