...
Antes de dar por cerrada una tarea que toca lógica de negocio (no solo UI), corré `npx tsc --noEmit` y `npm test` como mínimo; si tocás `db.ts`/`backup.ts`/flujos financieros o de permisos, agregá o extendé pruebas en `tests/` en vez de solo probar a mano.
...
- El nombre visible de la app (ventana, ícono/acceso directo de escritorio, instalador, nombre del release en GitHub) es siempre **"Clio"** — nunca "catalogo-imprenta"/"Catálogo Imprenta". `productName` y el título de ventana en `tauri.conf.json`, y `releaseName` en `.github/workflows/build.yml`, deben decir "Clio"; no revertirlos. El `identifier` (`com.mariat.catalogo-imprenta`) y el nombre del repo de GitHub quedan igual a propósito — son internos, invisibles para el usuario, y cambiarlos rompe la continuidad del auto-update para quien ya tiene la app instalada.
